...
export class ChairBusComponent implements OnInit {
  ...
  constructor() { }
  statusChair(chair){
    console.log(!chair.dangChon);
    if (chair.TrangThai){
      return(
        "btn-chair btn btn-danger mr-2 mb-2"
      )
    }else{
      if(chair.chose == undefined || chair.chose == false){
      return " btn-chair btn btn-secondary mr-2 mb-2"
    }else{
      return " btn-chair btn btn-success mr-2 mb-2"
      }
    }
  }
  changeChose(){
